...
public class ParticipantReportProcessor : IReportProcessorUnit<ParticipantReport>
{
    ...
    public async Task ProcessReportAsync(ParticipantReport report, CancellationToken token)
    {
        ...
        var friends = await _participantsGetterService.GetFriendsAsync(info, report.VkId, 500, token);
        var subscriptions = await _participantsGetterService.GetSubscriptionsAsync(info, report.VkId, 200, token);
        foreach (var participantDto in friends.Concat(subscriptions))
        {
